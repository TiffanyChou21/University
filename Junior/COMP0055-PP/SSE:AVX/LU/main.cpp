...
void reset_mat(float test[][N], float mat[][N])  //重置一致
{
    for (int i = 0; i < N; i++)
        for (int j = 0; j < N; j++)
            test[i][j] = mat[i][j];
...
void naive_lu(float mat[][N])     //朴素串行
{
    for (int k = 0; k < N; k++)
    {
        for (int j = k + 1; j < N; j++)
        ...
        {
            for (int j = k + 1; j < N; j++)
                mat[i][j] -= mat[i][k] * mat[k][j];//减去第k行
            mat[i][k] = 0;
        }
    }
}

void sse_lu(float mat[][N])   //89向量化
{
    __m128 t1, t2, t3;
    for (int k = 0; k < N; k++)
    {
...
void sse_lu_aligned(float mat[][N])//对齐
{
    __m128 t1, t2, t3;
    for (int k = 0; k < N; k++)
    {
...
void sse_lu_vectorized(float mat[][N])//4589
{
    __m128 t1, t2, t3;
    for (int k = 0; k < N; k++)
    {
...
void avx_lu_aligned(float mat[][N])//对齐
{
    __m256 t1, t2, t3;
    for (int k = 0; k < N; k++)
    {
